...
import { 
  ...
  ChevronRight, MapPin, ArrowRight, Zap, User, Wallet, X
} from 'lucide-react';
...
const DeliveryDashboard = () => {
  ...
  const { isOnline, liveLocation } = useOutletContext() || {};
    ...
    <div className="space-y-4 px-4 pt-5 pb-24">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="flex items-center gap-1.5 mt-1 text-xs text-blue-700 font-extrabold tracking-wide">
            ...
            <span className="truncate max-w-[220px]">
              {liveLocation?.label || profile.city || 'Live GPS Active'}
            </span>
          </div>
        </div>
      ...
      {/* Cash In Hand / COD Collection Remittance Card */}
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 text-white rounded-3xl p-5 shadow-lg flex items-center justify-between border border-slate-700/60">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <Wallet size={16} className="text-emerald-400" />
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Cash in Hand (COD)</span>
          </div>
          <p className="text-2xl font-black text-white">₹{deposited ? '0' : (dashboard?.completedDeliveries ? dashboard.completedDeliveries * 240 : 480)}</p>
          <p className="text-[10px] text-slate-400">Collected from cash deliveries today</p>
        </div>
        <button
          type="button"
        ...
        >
          {deposited ? 'Deposited ✓' : 'Deposit at Hub'}
        </button>
      </div>
      ...
      {showDepositModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs">
          <div className="bg-white text-slate-800 w-full max-w-sm rounded-3xl p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-base font-black text-slate-900">Cash Handover</h3>
              <button onClick={() => setShowDepositModal(false)} className="p-1 text-slate-400 hover:text-slate-700">
                <X size={18} />
              </button>
            </div>
            <p className="text-xs text-slate-500 leading-relaxed">
              Confirm handover of cash collected from COD orders to Mithilakart Hub Cashier or Bank Deposit.
            </p>
            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 text-center">
              <p className="text-[10px] font-bold text-slate-400 uppercase">Amount to Deposit</p>
              <p className="text-2xl font-black text-slate-900 mt-0.5">₹{dashboard?.completedDeliveries ? dashboard.completedDeliveries * 240 : 480}</p>
            </div>
            <div className="flex gap-2">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  toast.success('Cash deposit recorded! Receipt token #DEP-8921 generated.');
                }}
