...
export const mapDeliveryPartner = (p = {}) => ({
  id: p._id || p.id || '',
  name: p.name || 'Agent',
  zone: p.zone || p.serviceArea || p.city || '—',
  phone: p.phone ? `${p.countryCode || '+91'} ${p.phone}` : '—',
  status: p.isOnline ? 'Active' : titleCaseStatus(p.status || 'offline'),
  isOnline: Boolean(p.isOnline),
  codDuesBalance: p.codDuesBalance ?? 0,
  totalCodCollected: p.totalCodCollected ?? 0,
  balance: p.balance ?? 0,
  rating: p.rating ?? 0,
...
export const mapCategory = (c = {}) => ({
  ...
  visibleTabs: c.visibleTabs?.length ? c.visibleTabs : ['mithilakart'],
});
