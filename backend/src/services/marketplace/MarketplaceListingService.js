...
class MarketplaceListingService extends BaseService {
  ...
  async createListing(sellerId, productId, data) {
    ...
    if (masterStatus !== PRODUCT_STATUS.APPROVED && masterStatus !== PRODUCT_STATUS.PENDING) {
      throw AppError.conflict('Master product must be approved or pending before creating listings', [
    ...
    const isApprovedProduct = product.status === PRODUCT_STATUS.APPROVED;
    ...
    const listing = await this.marketplaceListingRepository.create({
      ...
      listingStatus: isApprovedProduct ? LISTING_STATUS.APPROVED : LISTING_STATUS.DRAFT,
      isVisible: isApprovedProduct,
      publishedAt: isApprovedProduct ? new Date() : null,
      approvedAt: isApprovedProduct ? new Date() : null,
  ...
  async listPublicForTab(query = {}) {
    ...
    if (query.categoryId) {
      ...
      const products = await this.productRepository.findPublic({ categoryId: { $in: categoryIds } }, { limit: 500 });
