...
function createDeliveryRoutes(controllers, middleware) {
  ...
  router.get('/orders/:id', ...deliveryScope, validateParams(Joi.object({ id: objectIdSchema })), controllers.orders.getOrderDetail);
  ...
  router.post('/orders/:id/reject', ...deliveryScope, validateParams(Joi.object({ id: objectIdSchema })), validateBody(Joi.object({ reason: Joi.string().trim().max(500).optional().allow('', null) })), controllers.orders.rejectOrder);
  router.post('/orders/:id/failed', ...deliveryScope, validateParams(Joi.object({ id: objectIdSchema })), validateBody(Joi.object({ reason: Joi.string().trim().max(500).optional().allow('', null) })), controllers.orders.markDeliveryFailed);
