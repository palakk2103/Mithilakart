...
function createOrdersRoutes({ orderController, returnController }, middleware) {
  ...
  router.get('/:id', authCustomer, validateParams(Joi.object({ id: objectIdSchema })), orderController.getOrderDetail);
  router.get('/:id/tracking', authCustomer, validateParams(Joi.object({ id: objectIdSchema })), orderController.getOrderTracking);
  router.post('/:id/cancel', authCustomer, validateParams(Joi.object({ id: objectIdSchema })), validateBody(cancelOrderSchema), orderController.cancelOrder);
  router.post('/:id/returns', authCustomer, validateParams(Joi.object({ id: objectIdSchema })), validateBody(Joi.object({
